...
#ifdef DEBUG_CRC
#define DPRINTF(fmt, ...) \
do { char *dtmp = get_datetime(); fprintf(stderr, "[%s ", dtmp); free(dtmp); dtmp=NULL; fprintf(stderr, "archiver/crc      ] " fmt , ## __VA_ARGS__); fflush(stderr); } while (0)
#else
#define DPRINTF(fmt, ...) \
do {} while(0)
#endif
...
uint32_t crc32_file(char *filename, int chunkSize)
{
	...
	fd = open(filename, O_RDONLY);
		...
		size = chunkSize;
		DPRINTF("Getting %s CRC for 0x%"PRIx32" bytes\n", filename, size);
	}
	...
	while ((rc = read(fd, data, size)) > 0) {
		...
		DPRINTF("Block size %d bytes: 0x%" PRIx32 "\n", rc, ret);
		memset(data, 0, size);
	}
